...
function showDetails(s) {
    // Traffic light dot based on risk
    let dotColor;
    if (s.riskLevel === "High") dotColor = "red";
    else if (s.riskLevel === "Medium") dotColor = "orange";
    else if (s.riskLevel === "Low") dotColor = "yellow";
    else dotColor = "gray"; // fallback

    scamDetails.innerHTML = `
        <h2>${s.name}</h2>
        <p><strong>Category:</strong> ${s.category}</p>
        <p><strong>Risk:</strong> ${s.riskLevel} 
           <span style="display:inline-block; width:12px; height:12px; background-color:${dotColor}; border-radius:50%; margin-left:5px;"></span>
        </p>
        <p><strong>Details:</strong> ${s.details}</p>
        <p><strong>What to do:</strong> ${s.reportSteps}</p>
        <p><strong>Report Website:</strong> ${s.reportWebsite}</p>
        <p><strong>Contact Number:</strong> ${s.contactNumber}</p>
    `;
}
